...
- **Sync-script validation**: `claude-diff-with-home` is a thin wrapper for `claude-sync-to-home --dry-run`; all plan/compare logic lives in `claude-sync-to-home`. After changing sync behavior (managed dirs, excludes, merge, local overrides), run `for script in claude-sync-to-home tests/sync-home.sh; do bash -n "$script"; done` and `./tests/sync-home.sh` (uses a temporary HOME; never touches `~/.claude`).
