...
- **Scope context-specific rules with `paths:`**: a rule in `home/rules/` that only applies in a narrow context (for example, Python work) should carry `paths:` frontmatter so it loads only when a matching file is **Read**; keep truly global rules unscoped. The trigger is Read of an existing file only — Write of a new file, Edit, and Bash-created files do not fire it — so scope a rule only if its use case starts by reading a matching file (Python: yes; AGENTS.md/CLAUDE.md authoring: no, it stays global because the rule matters most when creating the file). `paths:` is the only supported field for rules, and `@import` loads eagerly (not lazily) — neither defers global rules. See [README.md](./README.md).
